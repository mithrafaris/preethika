...
exports.getCheckout = async (req, res) => {
  ...
  try {
    if (req.session.id) {
      ...
      const userList = await userDB.findOne({ email: req.session.userId });
      ...
      if (user) {
        ...
        const uniqueCartItems = cartData.filter(
          (item, index, self) =>
            index ===
            self.findIndex(
              (t) => t.product && t.product._id.equals(item.product._id)
            )
        );
        ...
        res.render("checkout", {
          user: userList,
          addressList: userList.address.items,
          productData: uniqueCartItems,
          total: sub,
...
exports.getConfirmOrder = async (req, res) => {
  try {
    const user = await userDB.findOne({ email: req.session.userId });
    res.render("successTick", { user: user });
  } catch (err) {
